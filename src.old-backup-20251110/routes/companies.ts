...
router.get('/:id/employees', async (req, res, next) => {
  try {
    ...
    // Check ownership
    const accountOwnerId = await getAccountOwnerId(req.user!.id);
    if (company.ownerId !== accountOwnerId && company.ownerId !== req.user!.id) {
    ...
    // Import Proxycurl service
    const proxycurlService = await import('../services/proxycurl.service');

    // Fetch employees from Proxycurl
    const employees = await proxycurlService.fetchCompanyEmployees(company.linkedin, {
    ...
    if (error.message?.includes('credits exhausted')) {
      return res.status(402).json({
        error: 'Employee data quota exceeded',
        message: 'Please upgrade your plan or try again next month'
      });
    }
...
router.post('/:id/employees/import', async (req, res, next) => {
  try {
    ...
    for (const linkedinUrl of employeeUrls) {
      try {
        // Get person profile from Proxycurl
        const profile = await proxycurlService.getPersonProfile(linkedinUrl);
        ...
        const contact = await prisma.contact.create({
          data: {
            ...
            notes: `Imported from LinkedIn via Proxycurl on ${new Date().toISOString()}`
          }
        });
...
// GET /api/companies/proxycurl/credits - Check Proxycurl API credits
router.get('/proxycurl/credits', async (req, res, next) => {
  try {
    // Check if user is account owner
    const accountOwnerId = await getAccountOwnerId(req.user!.id);
    if (accountOwnerId !== req.user!.id) {
      return res.status(403).json({ error: 'Only account owners can check API credits' });
    }
    ...
    // Check credits
    const creditInfo = await proxycurlService.checkCredits();

    res.json(creditInfo);
  } catch (error: any) {
    console.error('Error checking Proxycurl credits:', error);

    if (error.message?.includes('PROXYCURL_API_KEY')) {
      return res.status(503).json({
        error: 'Proxycurl API not configured',
        message: 'Please add PROXYCURL_API_KEY to environment variables'
      });
    }
