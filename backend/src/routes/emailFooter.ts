...
/**
 * Email Footer Configuration API
 *
 * Manages centralized email footer settings for users
 * - GET /email-footer - Get user's footer config (or default)
 * - POST /email-footer - Create new footer config
 * - PUT /email-footer/:id - Update footer config
 * - DELETE /email-footer/:id - Delete footer config
 * - GET /email-footer/render - Get rendered HTML footer
 */

// Apply authentication middleware to all routes
router.use(authenticate);

// GET user's footer configuration (or create default if none exists)
router.get('/', async (req: Request, res: Response) => {
  try {
    ...
    // Try to find user's footer config
    let footerConfig = await prisma.emailFooterConfig.findFirst({
      where: {
        userId,
        isActive: true
      },
      orderBy: {
        isDefault: 'desc' // Prioritize default config
      }
    });

    // If no config exists, create default one
    if (!footerConfig) {
      footerConfig = await prisma.emailFooterConfig.create({
        data: {
          userId,
...
// GET all footer configs for user
router.get('/all', async (req: Request, res: Response) => {
  try {
    ...
    const configs = await prisma.emailFooterConfig.findMany({
      where: {
        userId
      },
      orderBy: {
...
router.post('/', async (req: Request, res: Response) => {
  try {
    ...
    const footerConfig = await prisma.emailFooterConfig.create({
      data: {
        userId,
        ...req.body
      }
...
router.put('/:id', async (req: Request, res: Response) => {
  try {
    ...
    // Verify ownership
    const existing = await prisma.emailFooterConfig.findUnique({
      where: { id }
    });
    ...
    const updated = await prisma.emailFooterConfig.update({
      where: { id },
      data: req.body
    });
...
router.delete('/:id', async (req: Request, res: Response) => {
  try {
    ...
    await prisma.emailFooterConfig.delete({
      where: { id }
    });
...
router.get('/render', async (req: Request, res: Response) => {
  try {
    ...
    const footerConfig = await prisma.emailFooterConfig.findFirst({
      where: {
        userId,
        isActive: true
      },
...
// Helper function to generate footer HTML from config
function generateFooterHTML(config: any): string {
  const {
    companyName, missionStatement, supportEmail, supportPhone, address,
    linkedinUrl, twitterUrl, facebookUrl, instagramUrl, youtubeUrl,
    privacyPolicyUrl, termsOfServiceUrl, gdprUrl, cookiePolicyUrl, dpaUrl,
    productLinks, resourceLinks, companyLinks,
    showSocialMedia, showMission, showQuickLinks, showContactInfo, showLegalLinks, showGdprNotice,
  ...
  if (showLegalLinks && (privacyPolicyUrl || termsOfServiceUrl || gdprUrl || cookiePolicyUrl || dpaUrl)) {
    ...
    const links = [];
    if (privacyPolicyUrl) links.push(`<a href="${privacyPolicyUrl}" style="color: ${primaryColor}; text-decoration: none; font-weight: 500;">Privacy Policy</a>`);
    if (termsOfServiceUrl) links.push(`<a href="${termsOfServiceUrl}" style="color: ${primaryColor}; text-decoration: none; font-weight: 500;">Terms of Service</a>`);
    if (gdprUrl) links.push(`<a href="${gdprUrl}" style="color: ${primaryColor}; text-decoration: none; font-weight: 500;">GDPR</a>`);
    if (cookiePolicyUrl) links.push(`<a href="${cookiePolicyUrl}" style="color: ${primaryColor}; text-decoration: none; font-weight: 500;">Cookie Policy</a>`);
    if (dpaUrl) links.push(`<a href="${dpaUrl}" style="color: ${primaryColor}; text-decoration: none; font-weight: 500;">DPA</a>`);
