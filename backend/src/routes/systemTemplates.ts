...
// Super admin email - only this user can access system templates
...
// GET /api/system-templates - List all system templates
...
// GET /api/system-templates/types - Get all template types
router.get('/types', async (req, res) => {
  const types = Object.entries(TEMPLATE_TYPES).map(([key, value]) => ({
    key,
    value,
...
// GET /api/system-templates/verified-emails - Get AWS SES verified emails
router.get('/verified-emails', async (req, res, next) => {
  try {
    // For now, return common verified emails from env
    // In production, you'd query AWS SES API for verified identities
...
// GET /api/system-templates/:id - Get single template
...
// POST /api/system-templates - Create new system template
router.post('/', async (req, res, next) => {
  try {
    ...
    // Create template
    const template = await prisma.systemEmailTemplate.create({
      data: {
        name: name.trim(),
        type: type.trim(),
...
// PUT /api/system-templates/:id - Update system template
router.put('/:id', async (req, res, next) => {
  try {
    ...
    const existingTemplate = await prisma.systemEmailTemplate.findUnique({
      where: { id },
    });
    ...
    if (type && type !== existingTemplate.type && type !== TEMPLATE_TYPES.CUSTOM) {
      const duplicate = await prisma.systemEmailTemplate.findFirst({
        where: {
          type,
          id: { not: id },
        },
    ...
    // Update template
    const template = await prisma.systemEmailTemplate.update({
      where: { id },
      data: {
...
// DELETE /api/system-templates/:id - Delete system template
router.delete('/:id', async (req, res, next) => {
  try {
    ...
    // Delete template
    await prisma.systemEmailTemplate.delete({
      where: { id },
    });
...
// POST /api/system-templates/:id/test - Send test email
...
// POST /api/system-templates/:id/duplicate - Duplicate a template
router.post('/:id/duplicate', async (req, res, next) => {
  try {
    ...
    const duplicate = await prisma.systemEmailTemplate.create({
      data: {
        name: `${template.name} (Copy)`,
        type: TEMPLATE_TYPES.CUSTOM, // Duplicates are always custom type
