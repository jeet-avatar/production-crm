  ...
  try {
    const response = await axios.post<ApolloSearchResponse>(
      ...
      payload,
      {
        ...
        timeout: 120000, // 120s — quick-6 raised from 10s default to cover Apollo enrichment loop pacing
      },
    );
  ...
  try {
    const response = await axios.post<{ person?: ApolloPerson } & ApolloPerson>(
      ...
      { id: personId, reveal_personal_emails: false },
      {
        ...
        timeout: 120000, // 120s — quick-6 long timeout for enrichment retries
      },
    );
