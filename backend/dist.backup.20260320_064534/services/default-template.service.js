...
exports.DefaultTemplateService = void 0;
...
© {{currentYear}} Critical River. All rights reserved.`;
    ...
    static async ensureDefaultTemplate(userId) {
        try {
            ...
            if (existingTemplate) {
                logger_1.logger.info(`User ${userId} already has default template`);
                return;
            }
            ...
            logger_1.logger.info(`Created default email template for user ${userId}`);
        }
        catch (error) {
            logger_1.logger.error('Error creating default template:', error);
        }
    }
    static async seedAllUsers() {
        try {
            ...
            for (const user of users) {
                ...
                created++;
                logger_1.logger.info(`Created default template for user: ${user.email}`);
            }
            logger_1.logger.info(`Default template seeding complete. Created: ${created}, Skipped: ${skipped}`);
            return { created, skipped };
        }
        catch (error) {
            logger_1.logger.error('Error seeding default templates:', error);
            throw error;
        }
    }
}
exports.DefaultTemplateService = DefaultTemplateService;
//# sourceMappingURL=default-template.service.js.map