...
const upload = (0, multer_1.default)({
    ...
    fileFilter: (req, file, cb) => {
        ...
        if (!allowedMimeTypes.includes(file.mimetype)) {
            logger_1.logger.warn(`Contact import rejected: invalid MIME type ${file.mimetype}`);
        ...
        if (!allowedExtensions.includes(fileExt)) {
            logger_1.logger.warn(`Contact import rejected: invalid extension ${fileExt}`);
...
router.post('/', async (req, res, next) => {
    try {
        const { firstName, lastName, email, phone, role, companyId, companyName, status, tagIds = [], } = req.body;
        const validation = (0, validation_1.validateContactData)({ firstName, lastName, email, phone });
...
function mapCSVFieldsToContact(headers) {
    const mapping = {};
    logger_1.logger.info(`[CSV Mapping] Processing ${headers.length} headers: ${headers.join(', ')}`);
    headers.forEach(header => {
        ...
        if (normalized.match(/^(email|e?mail|emailaddress|contactemail|workemail)$/)) {
            ...
            logger_1.logger.info(`[CSV Mapping] Unmapped field "${header}" -> custom field`);
        }
    });
    logger_1.logger.info(`[CSV Mapping] Mapping result: ${JSON.stringify(mapping, null, 2)}`);
    return mapping;
}
...
async function processCSVRecord(record, fieldMapping, userId, fileName) {
    ...
    logger_1.logger.info(`[CSV Import] Parsed contact: firstName="${contactData.firstName}", lastName="${contactData.lastName}", email="${contactData.email}", company="${companyData.name}"`);
    ...
    if (!hasContactInfo && !hasCompanyInfo) {
        logger_1.logger.warn(`[CSV Import] Skipping record - no contact or company information`);
        return { skipped: true };
    }
    if (!hasContactInfo && hasCompanyInfo) {
        logger_1.logger.info(`[CSV Import] Company-only record detected: "${companyData.name}"`);
        const companyId = await findOrCreateCompany(companyData, userId);
        logger_1.logger.info(`[CSV Import] Company created/found: ${companyId}`);
        return {
            companyId,
            companyOnly: true,
        };
    ...
    if (contactData.phone) {
        const phoneValidation = (0, validation_1.validatePhoneNumber)(contactData.phone);
        if (!phoneValidation.isValid) {
            logger_1.logger.error(`[CSV Import] Invalid phone for ${contactData.firstName}: ${phoneValidation.error}`);
            return {
                error: true,
    ...
    if (contactData.email) {
        const emailValidation = (0, validation_1.validateEmail)(contactData.email);
        if (!emailValidation.isValid) {
            logger_1.logger.error(`[CSV Import] Invalid email for ${contactData.firstName}: ${emailValidation.error}`);
            return {
                error: true,
    ...
    if (existing) {
        logger_1.logger.info(`[CSV Import] Duplicate contact found: ${contactData.email || contactData.firstName + ' ' + contactData.lastName}`);
        return {
            duplicate: true,
    ...
    const companyId = await findOrCreateCompany(companyData, userId);
    logger_1.logger.info(`[CSV Import] Company ID for ${contactData.firstName}: ${companyId || 'none'}`);
    const contact = await prisma.contact.create({
        data: {
            firstName: contactData.firstName,
            lastName: contactData.lastName,
