...
router.post('/invite', async (req, res, next) => {
    try {
        ...
        try {
            const { EmailService } = await Promise.resolve().then(() => __importStar(require('../services/google-smtp.service')));
...
router.put('/:userId/role', async (req, res, next) => {
    try {
        ...
        if (req.user?.teamRole !== 'OWNER') {
            throw new errorHandler_1.AppError('Only account owners can change team roles', 403);
        }
        const { userId } = req.params;
        const { teamRole } = req.body;
        if (!teamRole || !['MEMBER', 'ADMIN'].includes(teamRole)) {
            throw new errorHandler_1.AppError('Valid teamRole is required (MEMBER or ADMIN)', 400);
        }
        ...
        if (teamMember.accountOwnerId !== ownerId) {
            throw new errorHandler_1.AppError('You can only change roles for members of your own team', 403);
        }
        if (teamMember.teamRole === 'OWNER') {
            throw new errorHandler_1.AppError('Cannot change the role of account owners', 400);
        }
        const updatedUser = await app_1.prisma.user.update({
            where: { id: userId },
