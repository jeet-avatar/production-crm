...
const default_template_service_1 = require("../services/default-template.service");
...
router.post('/register', validateRegistration, async (req, res, next) => {
    try {
        ...
        default_template_service_1.DefaultTemplateService.ensureDefaultTemplate(user.id).catch((err) => {
            logger_1.logger.error('Failed to create default template on registration:', err);
        });
...
router.post('/login', validateLogin, async (req, res, next) => {
    try {
        ...
        const token = auth_1.AuthUtils.generateToken(user);
        default_template_service_1.DefaultTemplateService.ensureDefaultTemplate(user.id).catch((err) => {
            logger_1.logger.error('Failed to ensure default template on login:', err);
        });
