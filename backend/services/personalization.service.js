...
class PersonalizationService {
    async generateVideoScript(contactId, companyId, userCompanyName = 'BrandMonkz') {
        logger_1.logger.info('Generating personalized video script', { contactId, companyId });
    ...
    async generateEmailSubject(contactId, companyId, userCompanyName = 'BrandMonkz') {
        logger_1.logger.info('Generating personalized email subject', { contactId, companyId });
        const context = await this.buildContext(contactId, companyId, 'email_subject', userCompanyName);
        return this.generateContent(context);
    }
    async generateEmailBody(contactId, companyId, userCompanyName = 'BrandMonkz') {
        logger_1.logger.info('Generating personalized email body', { contactId, companyId });
        const context = await this.buildContext(contactId, companyId, 'email_body', userCompanyName);
        return this.generateContent(context);
    }
    async buildContext(contactId, companyId, campaignType, userCompanyName) {
    ...
    async generateContent(context) {
        ...
        if (company.intent)
            availableSignals.push('company_intent');
        if (company.hiringInfo)
            availableSignals.push('hiring_info');
        if (company.jobPostings)
            availableSignals.push('job_postings');
        if (company.aiRecentNews)
            availableSignals.push('recent_news');
        if (company.techStack)
            availableSignals.push('tech_stack');
        logger_1.logger.info('Available personalization signals', {
        ...
        try {
            const aiResponse = await (0, ai_helper_1.callClaudeWithFallback)({
                prompt: userPrompt,
                systemPrompt,
            ...
            personalizedElements.push(contact.firstName);
            if (aiResponse.includes(company.name))
                personalizedElements.push('company_name');
            ...
            logger_1.logger.info('Successfully generated personalized content', {
                campaignType,
                confidence,
            ...
            logger_1.logger.error('Personalization AI error, using fallback', { error: error.message });
            return {
    ...
    buildSystemPrompt(campaignType) {
...
Length: 50-80 words`;
    ...
    buildUserPrompt(context, availableSignals) {
        const { contact, company, campaignType, userCompany } = context;
        ...
        if (contact.role)
            prompt += `- Role: ${contact.role}\n`;
        prompt += `\nCOMPANY:\n`;
        prompt += `- Name: ${company.name}\n`;
        if (company.industry)
            prompt += `- Industry: ${company.industry}\n`;
        if (company.size)
            prompt += `- Size: ${company.size} employees\n`;
        if (company.location)
    ...
    calculateConfidence(signalCount) {
        if (signalCount >= 3)
            return 90;
        if (signalCount === 2)
            return 75;
