...
export interface SocialFlowEnrichmentData {
  ...
  employees?: string;
  enrichmentStatus?: {
    creditRating?: { success: boolean; error?: string };
    socialMedia?: { success: boolean; error?: string };
 ...
 */
function generateMockEnrichmentData(companyName: string): SocialFlowEnrichmentData {
  logger.info(`Using mock data for: ${companyName}`);
 ...
 */
export async function enrichCompanyWithSocialFlow(
  companyName: string,
  linkedinUrl?: string
): Promise<SocialFlowEnrichmentData> {
  try {
    logger.info(`Enriching company via SocialFlow: ${companyName}`);
    if (linkedinUrl) {
      logger.info(`  Using LinkedIn URL: ${linkedinUrl}`);
    }

    // If mock mode is enabled, return mock data immediately
    if (USE_MOCK_DATA) {
      logger.info('SOCIALFLOW_USE_MOCK is enabled - returning mock data');
    ...
    if (!response.ok) {
      if (response.status === 404) {
        logger.warn(`Company not found in SocialFlow: ${companyName}`);
        return {
          enrichmentStatus: {
    ...
    logger.info(`Processing SocialFlow data: company=${companyData.company_name}, industry=${companyData.industry}`);

    // Parse specialties - it comes as a single string with comma-separated values
    let technographics: string[] = [];
    ...
    logger.info(`Extracted ${technographics.length} specialties/technologies`);
    ...
    const socialFlowData: SocialFlowEnrichmentData = {
    ...
    logger.info(`SocialFlow enrichment successful for ${companyName}`);
    return socialFlowData;

  } catch (error: any) {
    logger.error(`❌ SocialFlow enrichment failed for ${companyName}:`, error);
