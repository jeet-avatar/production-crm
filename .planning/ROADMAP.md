...
**Goal:** Resolve three intertwined problems exposed during quick-8 live validation of the v3 branded shell sent to JM + Rajesh on 2026-05-31. (1) **Stream-incoherence:** The current STREAM_TEMPLATE_V3_BODY is structurally NetSuite-specific (preheader, header chrome "1000+ Implementations / NetSuite Practice", intro "We're a senior NetSuite and AI team", "A note on NetSuite Next 2026" callout, "01 NetSuite Practice / 02 ArthaBuild AI Your NetSuite copilot" service props, footer reprise) — but Phase 5 personalizes per Stream (the 9 real prod streams: NetSuite, AI/ML, Cloud/DevOps, Cybersecurity, Data/Analytics, Mobile, Enterprise/ERP, Staffing/HR, Other — verified at apollo.ts:73-76 VALID_STREAMS). A Cybersecurity-stream recipient sees a subject like "Security engineers — bench available" wrapped in NetSuite chrome, then 4 AI paragraphs about HIPAA + connected-device security; the email cannot tell what TechCloudPro is actually selling. Fix: 9 per-stream STREAM_TEMPLATE_V3_BODIES variants (one per Stream:* category, NO-SPACE dict keys matching prod DB email_templates.category values), each sharing the same visual chrome (navy header, orange accents, metrics row, Sara signature, mailto footer) but with stream-coherent COPY throughout (header tagline, intro, value props all aligned with that stream's domain) AND a unique per-stream HTML-comment sentinel `<!-- STREAM_V3:<bareStream> -->` for idempotency detection. Idempotent upgrade endpoint extends to be stream-aware. Plan 06-01 is split into 06-01a (human-readable copy review markdown) + 06-01b (TypeScript synthesis post-`copy-approved` checkpoint) — per the deploy-gate spirit, prospect-facing email bodies aren't auto-synthesized into code without user copy review. (2) **Human-in-the-loop review gate:** Even with per-stream coherence, AI-generated callout paragraphs will always have non-zero error rate. Introduce a `personalized_email_sends.status='pending_review'` value alongside existing `preview / sent / failed`. New body param on POST /api/apollo/send-personalized-campaign: `requireReview: boolean` — when true, personalize + render + persist with status='pending_review', skip Resend dispatch. New backend routes: GET /api/apollo/pending-review (list queue, ordered by createdAt), POST /api/apollo/pending-review/:id/approve (dispatch via Resend, flip status to sent), POST /api/apollo/pending-review/:id/reject (mark rejected with optional reason), POST /api/apollo/pending-review/:id/edit (override aiTokens; backend re-renders renderedBody server-side from updated tokens so Edit preview reflects the change). New frontend "Pending Review" tab on CampaignsPage showing inbox-card previews with per-row Approve / Edit / Reject buttons + bulk "Approve all on this page". (3) **Apollo Campaign entry-point button:** New button labeled "Apollo Campaign" placed BEFORE the existing "Create Campaign" button at the top-right of /campaigns page header. Click → fetch all source='apollo' contacts that have no successful personalized_email_sends row (status='sent') → group by stream (the 9 real prod streams) → per-group static lookup via `emailTemplatesApi.findByCategory('Stream:'+streamName)` (NO space) → per-group POST /send-personalized-campaign with requireReview:true → switch active tab to Pending Review. Respects existing 50-contact hard gate (confirmedLargeBatch:true required for N>50). Phase 4 firewall (campaigns.ts + awsSES.ts byte-clean vs phase-04-baseline tag) and Phase 5 send-path firewall (POST /api/apollo/send-personalized-campaign signature additive only — new optional requireReview field, existing callers continue working) both preserved.
**Depends on:** Phase 5
**Requirements:** REQ-060 (9 per-stream v3 bodies + stream-aware upgrade endpoint, NO-SPACE dict keys matching prod streams, HTML-comment sentinels for cross-contamination-proof idempotency), REQ-061 (personalized_email_sends.status='pending_review' enum value + Prisma migration), REQ-062 (requireReview body param on send-personalized-campaign route — additive, default false), REQ-063 (5 new backend routes: GET /unsent-contacts, GET /pending-review, POST /:id/approve, POST /:id/reject, POST /:id/edit with server-side re-render when aiTokens changes), REQ-064 ("Pending Review" tab on /campaigns with inbox-card previews + per-row actions + bulk approve + Edit modal showing re-rendered preview after save), REQ-065 ("Apollo Campaign" entry-point button on /campaigns header positioned before "Create Campaign", queue-staging flow with VALID_STREAMS_FRONTEND guard against rogue category values), REQ-066 (live verification: re-send to JM + Rajesh from each of the 9 streams, confirm coherent chrome end-to-end via per-stream HTML-comment sentinel positive + cross-contamination negative checks, Rajesh approves at least 1 via the queue UI BEFORE executor finishes remainder via curl)
**Plans:** 7 plans
...
- [x] 06-01b-PLAN.md — Wave 2 — **GATED COPY APPROVAL (autonomous: false, 1 checkpoint).** Task 1 = `copy-approved` checkpoint; Task 2 mechanically synthesizes STREAM_TEMPLATE_V3_BODIES (9-entry dict, NO-SPACE keys, HTML-comment sentinels) + stream-aware /upgrade-streams-v3; runtime guard prints OK 9/9 (REQ-060 step 2/2) — commit 01fe98a 2026-06-01
- [x] 06-03-PLAN.md — Wave 2 — POST /send-personalized-campaign extended with optional `requireReview` body field; old callers unchanged (REQ-062) — commit 3e38d37 2026-06-02
- [x] 06-04-PLAN.md — Wave 3 — 5 new backend routes (GET /unsent-contacts, GET /pending-review, POST /pending-review/:id/{approve,reject,edit}). /edit re-renders renderedBody server-side when aiTokens changes (closes stale-preview bug). Pre-flight verifies PersonalizedEmailSend.renderedBody String + .fromEmail String exist on schema (REQ-063) — commit d8b3350 2026-06-02
- [x] 06-05-PLAN.md — Wave 4 — Frontend Apollo Campaign button + Pending Review tab + apolloApi.unsentContacts + apolloApi.pendingReview.{list,approve,reject,edit} + PendingReviewQueue.tsx component. Uses static emailTemplatesApi.findByCategory('Stream:'+streamName) (NO space, NO dynamic import, NO placeholder). VALID_STREAMS_FRONTEND guard routes rogue stream values to 'Other' (REQ-064, REQ-065) — commit 9bb3311 2026-06-01
- [ ] 06-06-PLAN.md — Wave 5 — **GATED DEPLOY (autonomous: false, 2 checkpoints).** Checkpoint 1 = Deploy Approval; Task 2 deploys backend+frontend dist to EC2, applies migration, upgrades 9 templates to per-stream v3, generates STREAM-SAMPLES.json with per-stream HTML-comment sentinel positive + cross-contamination negative checks (~$0.62 Claude). Checkpoint 2 = Live Verify Approval; Task 4 stages 9-stream queue (~$0.62 Claude), Rajesh approves ≥1 via UI FIRST, executor approves remainder via curl ($0). Close ceremony writes 06-06-SUMMARY + 06-SUMMARY + STATE.md + ROADMAP.md updates (REQ-066). Total spend ~$1.24 Claude + $0 Resend.
